...
import {
  ...
  UserCog, CalendarCheck, Video, ExternalLink
} from "lucide-react";
import {
...
const seed = () => {
  const f1 = baseFields();
  const hcpId = uid();
  const mk = (n, e, p, st, note = "", hcp = "", grp = null, eq = {}, crm = {}) => ({
    id: uid(), state: st, note, flight: null, hcp, group: grp, eqChoice: eq, crm,
  ...
  const golf = {
    ...
    parts: [
      mk("Jan Novák",     "jan.novak@email.cz", "+420 601 234 567", "potvrzen", "VIP",  "12,4", "g1", { e1: true }, { category: "Velkoodběratel", reason: "Za poslední rok odebral GLE 450 a GLS 400. Dlouhodobý partner od 2018.", purchases: [{ year: 2024, model: "GLE 450" }, { year: 2024, model: "GLS 400" }, { year: 2022, model: "S 500" }] }),
      mk("Eva Dvořáková", "eva.d@firma.cz",      "+420 777 888 999", "potvrzen", "",    "18,2", "g1", { e2: true }, { category: "Obchodní partner", reason: "Jednatelka, firma odebírá fleet 8 vozů ročně. Zájem o elektrifikaci.", purchases: [{ year: 2023, model: "EQS 450" }, { year: 2023, model: "EQB 300" }] }),
      mk("Petr Svoboda",  "p.svoboda@mail.com",  "+420 602 111 222", "potvrzen", "",    "24,0", "g1", { e1: true }),
      mk("Karel Veselý",  "karel@golf.cz",       "+420 605 333 111", "prihlasen","",    "",     "g2", {}, { category: "Velkoodběratel", reason: "Ředitel výrobní firmy, pravidelný zákazník od 2019. Osobní vůz každé 2 roky, zájem o fleet pro firmu.", purchases: [{ year: 2024, model: "C 300" }, { year: 2023, model: "GLC 220d" }, { year: 2023, model: "A 200" }] }),
      mk("Lucie Malá",    "lucie.mala@firma.cz", "+420 720 444 222", "prihlasen","",    "",     "g2", {}, { category: "Společník", reason: "Společnice v advokátní kanceláři. První pozvání — navázání vztahu.", purchases: [] }),
      mk("Tomáš Horák",   "t.horak@email.cz",    "+420 608 555 333", "potvrzen", "",    "8,7",  "g2", { e2: true }, { category: "Dodavatel", reason: "Ředitel logistické firmy. Dodavatel náhradních dílů. Roční obrat spolupráce 2,4M.", purchases: [{ year: 2024, model: "Sprinter 319" }, { year: 2024, model: "Vito 116" }] }),
...
/* ════════════════════════════════════════
   DASHBOARD
════════════════════════════════════════ */
function Dashboard({ campaigns, role, used, onOpen, annualBudget, setAnnualBudget, totalExpected, totalReal, notifLog, onRemind, onCreate }) {
...
function Detail({ c, role, used, crossMap, blocked, onBack, onUpdate, onRemind }) {
  ...
  const addPart = (data, eqChoice = {}, customerInfo = {}) => {
    ...
    setDupErr("");
    onUpdate((camp) => ({ ...camp, parts: [...camp.parts, { id: uid(), state: "ceka", note: `Přidal: ${role}`, flight: null, hcp: "", group: null, eqChoice, crm: {}, customerInfo, data }] }));
    return true;
  };
    ...
    <div>
      ...
      {canApprove && waitCount > 0 && (
        <div style={{ background: `${T.purple}14`, border: `1px solid ${T.purple}44`, borderRadius: 10, padding: "11px 14px", marginBottom: 14 }}>
          ...
          {c.parts.filter((p) => p.state === "ceka").map((p) => (
  ...
  return (
    <>
      ...
      <div style={{ background: T.panel, border: `1px solid ${T.line}`, borderRadius: 11, overflow: "hidden" }}>
        {/* hlavička */}
        <div style={{ display: "grid", gridTemplateColumns: `2fr 1.7fr 1.2fr ${isGolf ? "0.7fr " : ""}1.3fr 1.2fr 1.1fr auto auto`, gap: 8, padding: "9px 12px", fontSize: 10.5, color: T.textDim, background: T.panel2, borderBottom: `1px solid ${T.line}`, letterSpacing: 0.3 }}>
          <span>JMÉNO / PROFIL</span><span>EMAIL</span><span>TELEFON</span>{isGolf && <span>HCP</span>}<span>SKUPINA</span><span>STAV</span><span>POZNÁMKA</span><span></span><span></span>
        </div>
        {c.parts.map((p) => {
          ...
          return (
            <React.Fragment key={p.id}>
              <div style={{ display: "grid", gridTemplateColumns: `2fr 1.7fr 1.2fr ${isGolf ? "0.7fr " : ""}1.3fr 1.2fr 1.1fr auto auto`, gap: 8, padding: "10px 12px", alignItems: "center", fontSize: 12, borderBottom: isExpanded ? "none" : `1px solid ${T.line}`, background: p.state === "ceka" ? `${T.purple}0a` : "transparent" }}>
                {/* jméno + CRM badge */}
                <span style={{ fontWeight: 500, display: "flex", alignItems: "center", gap: 5, flexWrap: "wrap" }}>
                  {p.data[nameId] || "—"}
                  {others.length > 0 && <span title={`Také na: ${others.join(", ")}`} style={{ fontSize: 10, color: T.brass, background: `${T.brass}18`, border: `1px solid ${T.brass}44`, padding: "1px 5px", borderRadius: 9 }}>+{others.length}</span>}
                  {crm.category && <span style={{ fontSize: 10, color: T.info, background: `${T.info}15`, border: `1px solid ${T.info}44`, padding: "1px 6px", borderRadius: 9, whiteSpace: "nowrap" }}>{crm.category}</span>}
                </span>
                <span style={{ color: T.creamDim, fontSize: 11.5 }}>{p.data[emailId] || "—"}</span>
                <span style={{ color: T.creamDim, fontSize: 11.5 }}>{p.data[phoneId] || "—"}</span>
                {isGolf && <span style={{ color: p.hcp ? T.cream : T.textDim, fontWeight: p.hcp ? 500 : 400, fontSize: 11.5 }}>{p.hcp || "—"}</span>}
                {canEdit
                  ? <select value={p.group || ""} onChange={(e) => setGroup(p.id, e.target.value || null)} style={{ ...inputStyle, padding: "4px 6px", fontSize: 11 }}>
                      <option value="">—</option>
                      {c.groups.map((g) => <option key={g.id} value={g.id}>{g.name}</option>)}
                    </select>
                  : <span style={{ fontSize: 11, color: T.textDim }}>{c.groups.find((g) => g.id === p.group)?.name || "—"}</span>
                }
                <select value={p.state} onChange={(e) => setState(p.id, e.target.value)} style={{ ...inputStyle, padding: "4px 5px", fontSize: 11 }} disabled={role === "lite"}>
                  {STATE_ORDER.filter((s) => !(role === "lite" && s === "ceka")).map((s) => <option key={s} value={s}>{STATES[s].label}</option>)}
                </select>
                <input value={p.note} placeholder="poznámka…" onChange={(e) => setNote(p.id, e.target.value)} style={{ ...inputStyle, padding: "4px 7px", fontSize: 11 }} readOnly={role === "lite"} />
                {/* Znovu odeslat */}
                {canApprove && (p.state === "prihlasen" || p.state === "schvaleno") && (
                  <button onClick={() => p.state === "schvaleno" ? alert("Zákazník čeká na hromadné odeslání.") : onResend(p.id)} title={p.state === "schvaleno" ? "Čeká na hromadné odeslání" : "Znovu odeslat pozvánku"} style={{ background: "none", border: `1px solid ${p.state === "schvaleno" ? T.greenLite : T.info}44`, borderRadius: 7, cursor: "pointer", padding: "4px 7px", color: p.state === "schvaleno" ? T.greenLite : T.info, fontSize: 11 }}>
                    {p.state === "schvaleno" ? "⏳ čeká" : "✉ znovu"}
                  </button>
                )}
              {/* CRM tlačítko */}
                <button onClick={() => toggleCrm(p.id)} title="Profil zákazníka" style={{ background: "none", border: `1px solid ${isExpanded || hasCrm ? T.info : T.line}`, borderRadius: 7, cursor: "pointer", padding: "4px 7px", color: isExpanded || hasCrm ? T.info : T.textDim, fontSize: 11, display: "flex", alignItems: "center", gap: 4 }}>
                  👤 {hasCrm ? "profil" : "+"}
                </button>
                {canEdit
                  ? <button onClick={() => remove(p.id)} style={{ background: "none", border: "none", cursor: "pointer", color: T.danger }}><Trash2 size={13} /></button>
                  : <span />
                }
              </div>
              {/* CRM rozbalovací panel */}
              {isExpanded && (
                <div style={{ padding: "0 12px 14px 12px", borderBottom: `1px solid ${T.line}`, background: `${T.info}06` }}>
...
function UsersModal({ users, onClose, onUpdate }) {
  ...
  return (
    <Modal title="Správa uživatelů" onClose={onClose} wide>
      ...
      {adding && <AddUserModal onClose={() => setAdding(false)} onAdd={(u) => { addUser(u); setAdding(false); }} />}
    </Modal>
  );
}
            ...
            <div style={{ marginBottom: 22 }}>
              ...
              <div style={{ background: T.bg, border: `1px solid ${T.line}`, borderRadius: 10, overflow: "hidden" }}>
                ...
                {c.team.members.map((m) => {
                  const [editFrom, setEditFrom] = React.useState(m.timeFrom || "08:00");
                  const [editTo,   setEditTo]   = React.useState(m.timeTo   || "18:00");
                  const st = TEAM_STATUSES[m.status] || TEAM_STATUSES.pending;
                  return (
                    <div key={m.id} style={{ display: "grid", gridTemplateColumns: "2fr 1.2fr 1fr 1fr 1.2fr", gap: 8, padding: "9px 12px", alignItems: "center", fontSize: 12.5, borderBottom: `1px solid ${T.line}` }}>
                      <div>
